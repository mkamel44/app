...
class SubMenuComponentState extends State<SubMenuComponent> {
  ...
  Widget build(BuildContext context) {
    return ExpansionTile(
      ...
      trailing:
          widget.mMenuList!.children!.isEmpty ? SizedBox() : Icon(isParentSelect == false ? Icons.keyboard_arrow_right : Icons.keyboard_arrow_down_rounded, color: Theme.of(context).iconTheme.color!.withOpacity(0.5)),
      tilePadding: EdgeInsets.all(0),
      childrenPadding: EdgeInsets.all(0),
      leading: appStore.isDarkModeOn == true
          ? cachedImage(widget.mMenuList!.image, width: 22, height: 22, fit: BoxFit.fitWidth, color: Theme.of(context).iconTheme.color!.withOpacity(0.5))
          : cachedImage(widget.mMenuList!.image, width: 22, height: 22, fit: BoxFit.fitWidth),
      children: <Widget>[
